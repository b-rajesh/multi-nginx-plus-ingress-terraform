...
resource "null_resource" "push-image-to-acr" {
  ...
  provisioner "local-exec" {
    command = "docker push ${azurerm_container_registry.nginx-ingress-acr.login_server}/${var.name_of_app_protect_ingress_container}:${var.ingress_conroller_version}"
  }
  provisioner "local-exec" {
  ...
  provisioner "local-exec" {
    command = "docker rmi  ${azurerm_container_registry.nginx-ingress-acr.login_server}/${var.name_of_app_protect_ingress_container}:${var.ingress_conroller_version}"
  }
}
